...
def find_lark_cli(config=None) -> str | None:
    ...
    if found:
        return found
    for p in sorted(glob.glob(os.path.expanduser("~/.nvm/versions/node/*/bin/lark-cli")), reverse=True):
        if os.path.exists(p):
            return p
    return None
...
class LarkCli:
    ...
    def available(self) -> bool:
        return bool(self.binary and os.path.exists(self.binary) or (self.binary and shutil.which(self.binary)))
    ...
    def whoami(self) -> dict | None:
        ...
        try:
            ...
            if apps:
                ...
                return {"app_id": apps[0].get("appId"), "brand": apps[0].get("brand"),
    ...
    def api(self, method: str, path: str, *, params: dict | None = None,
            data: dict | None = None, as_: str = "user", timeout: int = 120) -> dict:
        if not self.binary:
    ...
    def paged(self, method: str, path: str, *, params: dict | None = None,
              as_: str = "user", item_key: str = "items", max_pages: int = 50) -> list[dict]:
